 ...
 */
	...
	const SvgParser = require('./svgparser.js')
	const background = require("./background.js")	
	const GeometryUtil = require('./util/geometryutil.js');
	const ClipperLib = require('./util/clipper.js');
	const simplify = require('./util/simplify.js');
	function DeepNest(ipcrenderer){
		var self = this;
		...
		this.importsvg = function(filename, dirpath, svgstring, scalingFactor, dxfFlag){
			...
			console.log(4444,(svg.children[0]))
			svg = SvgParser.clean(dxfFlag, svg);
			console.log(5555,(svg.children[0]))
			if(filename){
				this.imports.push({
					filename: filename,
					svg: svg
			...
			console.log("parts",svg.children[0])
		...
		this.getParts = function(paths, filename){
			...
			for(i=0; i<numChildren; i++){
				...
				console.log(2222,paths[i])
				var poly = SvgParser.polygonify(paths[i].children[0]);
		...
		this.launchWorkers = function(parts, config, progressCallback, displayCallback){
          ...
          var filenames = [];
            ...
            filenames[j] = filename;
					...
					// background.onload();
					this.ipcrenderer.emit('background-start', {index: i, sheets: sheets, sheetids: sheetids, sheetsources: sheetsources, sheetchildren: sheetchildren, individual: GA.population[i], config: config, ids: ids, sources: sources, children: children, filenames: filenames});

					// ({index: i, sheets: sheets, sheetids: sheetids, sheetsources: sheetsources, sheetchildren: sheetchildren, individual: GA.population[i], config: config, ids: ids, sources: sources, children: children, filenames: filenames});
					running++;					
				}
			}
		}
