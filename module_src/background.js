...
window.onload = function (ipcrenderer) {
	// const { ipcRenderer } = require('electron');
	window.ipcrenderer = ipcrenderer;
	const path = require('path');
	window.addon = require(path.join(__dirname,'../build/Release/addon.node'));
	...
	ipcrenderer.on('background-start', (data) => {
		...
		console.log('pairs: ',pairs.length);
		  ...
		  function sync(){
			...
			console.log()
            ipcrenderer.emit('test', [data.sheets, parts, data.config, index]);
		  	var placement = placeParts(data.sheets, parts, data.config, index);
	
			placement.index = data.index;
			window.ipcrenderer.emit('background-response', placement);
		  }
...
function placeParts(sheets, parts, config, nestindex){
	...
	window.ipcrenderer.emit('background-progress', {index: nestindex, progress: -1});
