...
const rt = {};
 (function(root){
	...
	SvgParser.prototype.load = function(dirpath, svgString, scale, scalingFactor){
		...
		const dom = new jsdom.JSDOM(svgString);
		var svg = dom.window; //parser.parseFromString(svgString, "image/svg+xml");
		this.dirPath = dirpath;
		// var failed = svg.window.documentElement.nodeName.indexOf('parsererror')>-1;
		// if(failed){
		...
		if(svg){
			...
			// console.log(svg.document.body.children[0])
			var root = svg.document.body;;
			
			this.svg = svg.document.body;
			// console.log("root", root)
			this.svgRoot = root;
			console.log(321,this.svgRoot.children)
			...
			console.log(43,this.svgRoot.children)
			...
			if(!width || !viewBox){
				if(!scalingFactor){
					...
					console.log(3,this.svgRoot.children)

					root.setAttribute('transform', transform);
					
					this.conf.scale *= scalingFactor;
					console.log(5,this.svgRoot.children)
			...
			if(!width || viewBox.length < 4){
				console.log(4,this.svgRoot.children)
	...
	SvgParser.prototype.cleanInput = function(dxfFlag, svg){
		
		console.log(1,this.svgRoot.children[0])

		this.svgRoot = svg;
		console.log(2,this.svgRoot.children)
		...
		console.log(8,this.svgRoot.children[0]);

		// remove any g elements and bring all elements to the top level
		// this.svgRoot.parentElement = null;
		this.flatten(this.svgRoot, 0);
		console.log(8,this.svgRoot.children[0]);
	...
	SvgParser.prototype.applyTransform = function(element, globalTransform, skipClosed, dxfFlag){
		...
		// console.log('start')
		// console.log(element, element.getAttribute('transform'))
	...
	SvgParser.prototype.flatten = function(element, dept){
		// element = element.body
		for(var i=0; i<element.children.length; i++){
			this.flatten(element.children[i], dept+1);
		}
		console.log(9,element.outerHTML);

		if(element.tagName != 'svg' && element.parentElement && dept > 0){
			console.log("parent")
			while(element.children.length > 0){
				console.log("adding", element.children[0].outerHTML, element.children[0].tagName, dept)
				element.parentElement.appendChild(element.children[0]);
			}
		}
		console.log(10,element.outerHTML);
	...
	SvgParser.prototype.splitPath = function(path){
		...
		var lastM = 0;
		console.log(path, path.tagName, path.getAttribute('d'))
		seglist = path.getAttribute('d')
